...
If the CSS for `.button` includes rules like `:disabled`, it will fail to apply correctly to the `<a>` and `<div>` elements. The contract was weak because the class name only described a visual intent, not the underlying semantics, accessibility, and behavior of the element itself. The contract is with the developer's naming scheme, not with the platform.
...
Flesh Cage's architecture is built on this premise. Within the encapsulated safety of a Shadow DOM, we can shed the defensive conventions of the past and embrace simple, readable, and powerful type selectors. A skin for a `<styled-button>` can safely and unambiguously target `button` because it knows, with certainty, that it will only ever be styling the single `<button>` element defined in its component template.
...
- Instead of `.tab--is-active`, we style the `[aria-selected="true"]` attribute.
- Instead of `.accordion--is-open`, we style the `[aria-expanded="true"]` attribute.
...
This idea of separating a fixed structure from a swappable presentation is not new. It was proven with stunning clarity two decades ago by the **CSS Zen Garden**.
...
The result was a breathtaking gallery of visual diversity, all built on the same underlying structure, all built on the same underlying structure. One designer would submit a stylesheet that rendered the page as a serene, minimalist blog. Another would render it as a gritty, textured movie poster. The HTML never changed, but the user experience was completely different.
...
- Where the Zen Garden relied on IDs and classes for its styling hooks, Flesh Cage elevates the contract to the more robust and meaningful level of tags, attributes, and `::part` pseudo-elements.
