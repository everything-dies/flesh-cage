...
async function runBenchmark(type, count) {
  ...
  log(
    `Mount time: ${mountTime.toFixed(2)}ms (${(mountTime / count).toFixed(3)}ms per component)`
  )
  ...
  if (memAfterMount) {
    log(
      `Memory after mount: ${memAfterMount.used} MB used / ${memAfterMount.total} MB total`
    )
    log(
      `Memory increase: ${(memAfterMount.used - memBefore.used).toFixed(2)} MB`
    )
  }
  ...
  log(
    `Theme switch time: ${switchTime.toFixed(2)}ms (${(switchTime / count).toFixed(3)}ms per component)`
  )
  ...
  log(
    `Unmount time: ${unmountTime.toFixed(2)}ms (${(unmountTime / count).toFixed(3)}ms per component)`
  )
  ...
  if (memAfterUnmount) {
    log(
      `Memory after unmount: ${memAfterUnmount.used} MB used / ${memAfterUnmount.total} MB total`
    )
    log(
      `Memory retained: ${(memAfterUnmount.used - memBefore.used).toFixed(2)} MB`
    )
  }
  ...
  updateStats({
    ...
    avgMount: {
      ...
      value: (mountTime / count).toFixed(2),
      unit: 'ms',
    },
    switchTime: {
      label: 'Theme Switch',
      value: switchTime.toFixed(0),
      unit: 'ms',
    },
    avgSwitch: {
      label: 'Avg Switch Time',
      value: (switchTime / count).toFixed(2),
      unit: 'ms',
    },
    memory: {
      label: 'Memory Increase',
      value: memAfterMount
        ? (memAfterMount.used - memBefore.used).toFixed(1)
        : 'N/A',
      unit: 'MB',
    },
  })
...
async function runMemoryComparison() {
  ...
  for (const count of counts) {
    ...
    const increase = memAfter
      ? (memAfter.used - memBefore.used).toFixed(2)
      : 'N/A'
    ...
    const increase2 = memAfter2
      ? (memAfter2.used - memBefore2.used).toFixed(2)
      : 'N/A'
  ...
  counts.forEach((count) => {
    ...
    log(
      `${count.toString().padEnd(10)} | ${c.toString().padEnd(13)} | ${t.toString().padEnd(11)} | ${diff} MB`
    )
  })
...
async function runThemeSwitchBenchmark() {
  ...
  // Test constructable
  log(
    `Testing Constructable theme switching (${count} components, ${iterations} switches)...`
  )
  const container = document.getElementById('container')
  container.innerHTML = ''
  ...
  const constructableAvg =
    constructableTimes.reduce((a, b) => a + b, 0) / iterations
  ...
  const traditionalAvg =
    traditionalTimes.reduce((a, b) => a + b, 0) / iterations
  ...
  updateStats({
    ...
    constructable: {
      label: 'Constructable Avg',
      value: constructableAvg.toFixed(0),
      unit: 'ms',
    },
    traditional: {
      label: 'Traditional Avg',
      value: traditionalAvg.toFixed(0),
      unit: 'ms',
    },
    speedup: {
      label: 'Speedup',
      value: (traditionalAvg / constructableAvg).toFixed(1),
      unit: 'x',
    },
  })
...
log(
  'Note: For accurate memory measurements, run Chrome with --enable-precise-memory-info flag.'
)
