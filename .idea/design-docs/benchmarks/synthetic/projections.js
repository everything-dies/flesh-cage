...
function calculateMemory(scenario) {
  ...
  if (cacheStrategy === 'refCounting' || cacheStrategy === 'eternal') {
    ...
    constructableMemory.stylesheets = uniqueSheets * (CONSTANTS.STYLESHEET_OVERHEAD + cssSize);
  } else {
    // No caching: each instance has its own sheet (worst case)
    constructableMemory.stylesheets = totalInstances * (CONSTANTS.STYLESHEET_OVERHEAD + cssSize);
  }
  ...
  let traditionalMemory = {
    ...
    styleTags: totalInstances * (CONSTANTS.TRADITIONAL_STYLE_TAG_OVERHEAD + cssSize),
    // Shadow roots
    shadowRoots: totalInstances * CONSTANTS.SHADOW_ROOT_OVERHEAD,
    // Component instances
    instances: totalInstances * CONSTANTS.COMPONENT_INSTANCE_OVERHEAD,
  ...
  return {
    ...
    savingsPercent: ((traditionalMemory.total - constructableMemory.total) / traditionalMemory.total * 100).toFixed(1),
  };
}
...
function calculatePerformance(scenario) {
  ...
  if (cacheStrategy === 'eternal' || cacheStrategy === 'refCounting') {
    ...
    const firstMountPerComponent = CONSTANTS.STYLESHEET_REPLACE + CONSTANTS.STYLESHEET_ADOPTION;
    // Subsequent mounts: use cached sheets
    const cachedMountPerComponent = CONSTANTS.STYLESHEET_REPLACE_CACHED + CONSTANTS.STYLESHEET_ADOPTION;
    ...
    constructableMountTime = totalInstances * (CONSTANTS.STYLESHEET_REPLACE + CONSTANTS.STYLESHEET_ADOPTION);
  }
...
function printScenario(name, scenario) {
  ...
  console.log(`  Total Instances: ${scenario.uniqueComponents * scenario.instancesPerComponent}`);
  ...
  console.log(`    Stylesheets:  ${formatBytes(memory.constructable.stylesheets)}`);
  console.log(`    Shadow Roots: ${formatBytes(memory.constructable.shadowRoots)}`);
  console.log(`    Instances:    ${formatBytes(memory.constructable.instances)}`);
  ...
  console.log(`    Shadow Roots: ${formatBytes(memory.traditional.shadowRoots)}`);
  ...
  console.log(`    Constructable: ${performance.mount.constructable.toFixed(2)}ms`);
  console.log(`    Traditional:   ${performance.mount.traditional.toFixed(2)}ms`);
  console.log(`    Speedup:       ${performance.mount.speedup}x faster`);
  console.log(`\n  Theme Switch:`);
  console.log(`    Constructable: ${performance.themeSwitch.constructable.toFixed(2)}ms`);
  console.log(`    Traditional:   ${performance.themeSwitch.traditional.toFixed(2)}ms`);
...
const scenarios = {
  ...
  'Large App (Enterprise Design System)': {
    ...
    skins: ['light', 'dark', 'brand1', 'brand2', 'brand3', 'brand4', 'brand5', 'brand6', 'brand7', 'brand8'],
...
console.log('║' + ' '.repeat(20) + 'CONSTRUCTABLE STYLESHEETS PROJECTIONS' + ' '.repeat(21) + '║');
console.log('╚' + '═'.repeat(78) + '╝');
...
Object.entries(scenarios).forEach(([name, scenario]) => {
  const memory = calculateMemory(scenario);
  const performance = calculatePerformance(scenario);
  const totalInstances = scenario.uniqueComponents * scenario.instancesPerComponent;
